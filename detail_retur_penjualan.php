...
include 'db.php';
 ...
 $query = $db->query ("SELECT dp.no_faktur_penjualan, dp.no_faktur_retur, dp.id, dp.nama_barang, dp.kode_barang, 
 	...
 	WHERE dp.no_faktur_retur = '$no_faktur_retur'");
...
<table id="tableuser" class="table table-bordered">
		...
		<?php
			...
			while ($data1 = mysqli_fetch_array($query))
			{
				...
				$ss = $db->query("SELECT s.nama AS satuan_jual FROM detail_penjualan dp LEFT JOIN satuan s ON dp.satuan = s.id 
					WHERE dp.no_faktur = '$data1[no_faktur_penjualan]' AND dp.kode_barang = '$data1[kode_barang]' ");
